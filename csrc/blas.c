...
/* Thread-local B_pack buffer, allocated once on first use and freed when the
 * calling thread exits. Avoids aligned_alloc+free inside every sgemm call –
 * which adds up at ~2300 calls per long-audio encoder pass.
 *
 * Uses pthread_key_create with a destructor instead of `__thread` so the
 * buffer is reclaimed cleanly when threads die — important for long-lived
 * embedders (e.g. servers) that spin many short-lived caller threads.
 * Portable across Linux (libpthread) and macOS (libSystem). */
static pthread_key_t  tls_B_pack_key;
static pthread_once_t tls_B_pack_once = PTHREAD_ONCE_INIT;

static void tls_B_pack_destroy(void *p) { free(p); }

static void tls_B_pack_init(void)
{
    pthread_key_create(&tls_B_pack_key, tls_B_pack_destroy);
}

static float *get_B_pack(void)
{
    pthread_once(&tls_B_pack_once, tls_B_pack_init);
    float *p = (float *)pthread_getspecific(tls_B_pack_key);
    if (p == NULL) {
        p = (float *)aligned_alloc(64, B_PACK_BYTES);
        if (p != NULL)
            pthread_setspecific(tls_B_pack_key, p);
    }
    return p;
}
